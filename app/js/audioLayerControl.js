function audioLayerControl(elementContext)
{
    ...
    this.decodeAudioFinished = function decodeAudioFinished(audioBuffer)
    {
        ...
        var channelNames = ["Mono Channel", "Left Channel", "Right Channel"];
        
        if (numChannels == 1)
        {
            var editor = activeAudioLayerControl.createSequenceEditor(channelNames[0]);
            var sequence = CreateNewAudioSequence(sampleRate, audioBuffer.getChannelData(0));
            editor.setAudioSequence(sequence);
            editor.zoomToFit();
        }
        else{
        for (var i = 1; (i - 1) < numChannels; i++)
        {
            var editor = activeAudioLayerControl.createSequenceEditor(channelNames[i]);
            var sequence = CreateNewAudioSequence(sampleRate, audioBuffer.getChannelData(i - 1));
            editor.setAudioSequence(sequence);
            editor.zoomToFit();
        }
